...
public final class CcaComponentSubclassGenerator {
    ...
    public static Class<?> getClassFor(@MagicConstant(flagsFromClass = CcaComponentSubclassGenerator.class) int flags) {
        ...
        return PREGENERATED_CLASSES.computeIfAbsent(flags, unused -> {
            ...
            writer.visit(V1_8, ACC_PUBLIC | ACC_SUPER, className, null, "me/basiqueevangelist/commoncomponents/fabric/CcaComponent", gatherInterfacesFor(flags));
            writer.visitEnd();

            MethodVisitor mv = writer.visitMethod(ACC_PUBLIC, "<init>", "(Lme/basiqueevangelist/commoncomponents/Component;)V", null, new String[0]);
            mv.visitCode();
            mv.visitVarInsn(ALOAD, 0);
            mv.visitVarInsn(ALOAD, 1);
            mv.visitMethodInsn(INVOKESPECIAL, "me/basiqueevangelist/commoncomponents/fabric/CcaComponent", "<init>", "(Lme/basiqueevangelist/commoncomponents/Component;)V", false);
            mv.visitInsn(RETURN);
            mv.visitMaxs(1, 1);
            mv.visitEnd();
            ...
            gatherWrappedGettingMethods(flags, interfaceName -> {
                ...
                newMv.visitFieldInsn(GETFIELD, className, "wrapped", "Lme/basiqueevangelist/commoncomponents/Component;");
    ...
    private static String[] gatherInterfacesFor(@MagicConstant(flagsFromClass = CcaComponentSubclassGenerator.class) int flags) {
        ...
        if ((flags & SYNCING) == SYNCING)
            interfaces.add("me/basiqueevangelist/commoncomponents/fabric/CcaSyncingImpl");
        if ((flags & CLIENT_TICKING) == CLIENT_TICKING)
            interfaces.add("me/basiqueevangelist/commoncomponents/fabric/CcaClientTickingImpl");
        if ((flags & SERVER_TICKING) == SERVER_TICKING)
            interfaces.add("me/basiqueevangelist/commoncomponents/fabric/CcaServerTickingImpl");
    ...
    private static void gatherWrappedGettingMethods(@MagicConstant(flagsFromClass = CcaComponentSubclassGenerator.class) int flags, Consumer<String> consumer) {
        if ((flags & SYNCING) == SYNCING)
            consumer.accept("me/basiqueevangelist/commoncomponents/SyncingComponent");
        if ((flags & CLIENT_TICKING) == CLIENT_TICKING)
            consumer.accept("me/basiqueevangelist/commoncomponents/ClientTickedComponent");
        if ((flags & SERVER_TICKING) == SERVER_TICKING)
            consumer.accept("me/basiqueevangelist/commoncomponents/ServerTickedComponent");
    }
    ...
    private static String generateClassNameFor(@MagicConstant(flagsFromClass = CcaComponentSubclassGenerator.class) int flags) {
        ...
        sb.append("me/basiqueevangelist/commoncomponents/fabric/asm/generated/Cca");
        if ((flags & SYNCING) == SYNCING)
            sb.append("Syncing");
        if ((flags & CLIENT_TICKING) == CLIENT_TICKING)
